...
class CTHMM(MultiColor):
    """
    Hidden Markov model for multi color data with cross-talk.
    """
    ...
    def model(self):
        self.crosstalk = pyro.sample(
            "crosstalk_sample", dist.HalfNormal(torch.tensor([1.0, 1.0])).to_event(1)
        )
    ...
    def spot_model(self, data, prefix):
        ...
        # aoi sites
        aois = pyro.plate(f"{prefix}/aois", data.N, dim=-3)
        # time frames
        frames = (
            pyro.vectorized_markov(name=f"{prefix}/frames", size=data.F, dim=-2)
            if self.vectorized
        ...
        with aois as ndx:
            ...
            for fdx in frames:
                ...
                probs = (
                    ...
                    else Vindex(self.trans)[torch.arange(data.C), z_prev]
                )
                z_curr = pyro.sample(f"{prefix}/z_{fsx}", dist.Categorical(probs))
                theta = pyro.sample(
                    f"{prefix}/theta_{fsx}",
                    dist.Categorical(
                        Vindex(self.probs_theta)[z_curr, torch.arange(data.C)]
                    ),
                ...
                for kdx in spots:
                    ...
                    m = pyro.sample(
                        ...
                        dist.Categorical(
                            Vindex(self.probs_m)[theta, kdx, torch.arange(data.C)]
                        ),
                    )
                    with handlers.mask(mask=m > 0):
                        ...
                        x = pyro.sample(
                            ...
                            AffineBeta(
                                0,
                                Vindex(self.size)[ontarget, torch.arange(data.C)],
                                -(data.P + 1) / 2,
                                (data.P + 1) / 2,
                            ),
                        )
    ...
    def spot_guide(self, data, prefix):
        ...
        with aois as ndx:
            ...
            for fdx in frames:
                if self.vectorized:
                    fsx, fdx = fdx
                    if not isinstance(fdx, int):
                        fdx = fdx[..., None]
                    else:
                        fdx = torch.as_tensor(fdx).unsqueeze(-1).unsqueeze(-1)
                else:
                    fsx = fdx
                # sample background intensity
                pyro.sample(
                    f"{prefix}/background_{fsx}",
                    dist.Gamma(
                        Vindex(pyro.param(f"{prefix}/b_loc"))[
                            ndx, fdx, torch.arange(data.C)
                        ]
                        * Vindex(pyro.param(f"{prefix}/b_beta"))[
                            ndx, fdx, torch.arange(data.C)
                        ],
                        Vindex(pyro.param(f"{prefix}/b_beta"))[
                            ndx, fdx, torch.arange(data.C)
                ...
                z_probs = (
                    Vindex(pyro.param(f"{prefix}/z_trans"))[
                        ndx, fdx, torch.arange(data.C), 0
                    ]
                    if isinstance(fsx, int) and fsx < 1
                    # if isinstance(fdx, int) and fdx < 1
                    else Vindex(pyro.param(f"{prefix}/z_trans"))[
                        ndx, fdx, torch.arange(data.C), z_prev
                    ]
                )
                z_curr = pyro.sample(
                    f"{prefix}/z_{fsx}",
                ...
                for kdx in spots:
                    ...
                    m_probs = Vindex(pyro.param(f"{prefix}/m_probs"))[
                        z_curr, kdx, ndx, fdx, torch.arange(data.C)
                    ]
                    m = pyro.sample(
                        f"{prefix}/m_{kdx}_{fsx}",
                        ...
                        pyro.sample(
                            ...
                            dist.Gamma(
                                Vindex(pyro.param(f"{prefix}/h_loc"))[
                                    kdx, ndx, fdx, torch.arange(data.C)
                                ]
    ...
    def init_parameters(self):
        ...
        pyro.param(
            "init_mean",
            lambda: torch.ones(2, self.S + 1, device=device),
            constraint=constraints.simplex,
        )
        pyro.param(
            "init_size",
            lambda: torch.tensor([2, 2], device=device),
            constraint=constraints.positive,
        )
        pyro.param(
            "trans_mean",
            lambda: torch.ones(2, self.S + 1, self.S + 1, device=device),
            constraint=constraints.simplex,
        )
        pyro.param(
            "trans_size",
            lambda: torch.full((2, self.S + 1, 1), 2, device=device),
            constraint=constraints.positive,
        )

        pyro.param(
            "gain_loc",
            lambda: torch.tensor([5, 5], device=device),
            constraint=constraints.positive,
        )
        pyro.param(
            "gain_beta",
    ...
    def spot_parameters(self, data, prefix):
        ...
        pyro.param(
            ...
            lambda: torch.ones(data.N, 1, 2, device=device),
            constraint=constraints.positive,
        )
        ...
        pyro.param(
            f"{prefix}/b_beta",
            lambda: torch.ones(data.N, data.F, 2, device=device),
            constraint=constraints.positive,
        )
        pyro.param(
            f"{prefix}/h_loc",
            lambda: torch.full((self.K, data.N, data.F, 2), 2000, device=device),
            constraint=constraints.positive,
        )
        pyro.param(
            f"{prefix}/h_beta",
            lambda: torch.full((self.K, data.N, data.F, 2), 0.001, device=device),
            constraint=constraints.positive,
        )
        pyro.param(
            f"{prefix}/w_mean",
            lambda: torch.full((self.K, data.N, data.F, 2), 1.5, device=device),
            constraint=constraints.interval(
        ...
        pyro.param(
            f"{prefix}/w_size",
            lambda: torch.full((self.K, data.N, data.F, 2), 100, device=device),
            constraint=constraints.greater_than(2.0),
        )
        pyro.param(
            f"{prefix}/x_mean",
            lambda: torch.zeros(self.K, data.N, data.F, 2, device=device),
            constraint=constraints.interval(
        ...
        pyro.param(
            f"{prefix}/size",
            lambda: torch.full((self.K, data.N, data.F, 2), 200, device=device),
            constraint=constraints.greater_than(2.0),
        )
