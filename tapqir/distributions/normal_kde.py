...
class NormalKDE(TorchDistribution):
    ...
    def log_prob(self, value):
        ...
        my_routine = Genred(formula, variables, reduction_op='LogSumExp', axis=1, dtype=self.dtype)
        result = my_routine(value, self.logits, self.loc, self.var, self.p, backend=self.device)
