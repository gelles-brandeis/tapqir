...
class Modelv6:
    ...
    def __init__(self, data, dataset, K, lr, n_batch, jit, noise="GammaOffset"):
        ...
        pyro.get_param_store().load(os.path.join(data.path, "runs", dataset, "junk", "M2/lr/0.0003/h/10", "params"))
        self.epoch_count = 0
        self.lr = lr
    ...
    def gaussian_spot(self, batch_idx, height, width, x0, y0):
        ...
        for k in range(self.K):
            if k == 0:
                ...
                w = width[...,0,0]
                rv = dist.MultivariateNormal(spot_locs[...,0,0,:], scale_tril=self.spot_scale * w.view(w.size()+(1,1)))
