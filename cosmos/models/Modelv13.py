    ...
    def model(self):
        ...
        with N_plate as batch_idx:
            with F_plate:
                ...
                #with K_plate:
                m = pyro.sample("m", dist.Categorical(m_pi).expand([1,1,1,1,self.K]).to_event(1)) # N,F,1,1,K
                #theta = pyro.sample("theta", dist.Categorical(theta_pi)) # K,N,F,1,1
                height = pyro.sample("height", dist.Gamma(height_loc[m] * height_beta[m], height_beta[m]).to_event(1)) # N,F,1,1,K
                #width = pyro.sample("width", Location(1.3, 10., 0.5, 2.5))
                x0 = pyro.sample("x0", dist.Normal(torch.tensor(0.), scale))
                y0 = pyro.sample("y0", dist.Normal(torch.tensor(0.), scale))
        ...
        with N_plate as batch_idx:
            with F_plate:
                ...
                with K_plate:
                    ...
                    height = pyro.sample("height", dist.Gamma(h_loc[:,batch_idx], 1.))
                    #pyro.sample("width", Location(w_mode[:,batch_idx], w_size[:,batch_idx], 0.5, 2.5))
                    scale = torch.sqrt((width**2 + 1/12) / height + 8 * math.pi * width**4 * background / height**2)
                    pyro.sample("x0", dist.Normal(x_mode[:,batch_idx], scale))
                    pyro.sample("y0", dist.Normal(y_mode[:,batch_idx], scale))
