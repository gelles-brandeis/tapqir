...
class Modelv13:
    ...
    def model(self):
        ...
        with N_plate as batch_idx:
            with F_plate:
                ...
                theta = theta_matrix[theta] # N,F,1,1,K
                ...
                x0 = pyro.sample("x0", dist.Normal(torch.tensor(0.), scale[theta]).to_event(1))
                y0 = pyro.sample("y0", dist.Normal(torch.tensor(0.), scale[theta]).to_event(1))
    ...
    def guide(self):
        ...
        theta_probs = pyro.param("theta_probs", torch.ones(self.N,self.F,1,1,3), constraint=constraints.simplex)
        ...
        with N_plate as batch_idx:
            with F_plate:
                ...
                pyro.sample("theta", dist.Categorical(theta_probs[batch_idx])) # N,F,1,1
