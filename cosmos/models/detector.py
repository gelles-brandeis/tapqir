...
class Detector(Model):
    ...
    def __init__(self, data, control, K, lr, n_batch, jit, noise="GammaOffset"):
        ...
        #self._m = torch.load(os.path.join(self.data.path, "{}_m.pt".format(self.data.name)), map_location=self.data.device)
    ...
    def model(self):
        ...
        height_loc = pyro.sample("height_loc", dist.HalfNormal(torch.tensor([2000.])))
        height_beta = pyro.sample("height_beta", dist.HalfNormal(torch.tensor([10.])))
        #height_loc = pyro.sample("height_loc", dist.HalfNormal(torch.tensor([50., 2000.])).to_event(1))
            ...
            with pyro.plate("F_plate", self.data.F, dim=-4):
                ...
                with pyro.plate("K_plate", self.K, dim=-1):
                    with pyro.poutine.mask(mask=m.bool()):
                        #height = pyro.sample("height", dist.Gamma(height_loc[m] * height_beta[m], height_beta[m])) # N,F,1,1,K
                        height = pyro.sample("height", dist.Gamma(height_loc * height_beta, height_beta)) # N,F,1,1,K
    ...
    def guide(self):
        ...
        #pyro.sample("height_loc", dist.Delta(param("height_loc_v")).to_event(1))
        #pyro.sample("height_beta", dist.Delta(param("height_beta_v")).to_event(1))
        ...
        with pyro.plate("N_plate", self.data.N, subsample_size=self.n_batch, dim=-5) as batch_idx:
            with pyro.plate("F_plate", self.data.F, dim=-4):
                ...
                m = pyro.sample("m", dist.Categorical(param("m_probs")[batch_idx]))
                m = m_matrix[m.squeeze(dim=-1)] # N,F,1,1,K
                with pyro.plate("K_plate", self.K, dim=-1):
                    with pyro.poutine.mask(mask=m.bool()):
    ...
    def parameters(self):
        #pyro.get_param_store().load(os.path.join(self.data.path, "runs", self.data.name, "features/K{}".format(self.K), "lr{}".format(0.001), "params"))
        pyro.get_param_store().load(os.path.join(self.data.path, "runs", self.data.name, "marginal-detector/K{}".format(self.K), "lr{}".format(self.lr), "params"))

        # Parameters

        param("m_probs", torch.ones(self.data.N,self.data.F,1,1,1,4), constraint=constraints.simplex)
        # Data
