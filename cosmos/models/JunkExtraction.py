...
class JunkExtraction:
    ...
    def __init__(self, data, dataset, lr, jit, noise="GammaOffset"):
        ...
        self.writer = SummaryWriter(log_dir=os.path.join(self.data.path,"runs", "{}".format(self.dataset), "junks"))
    ...
    def model(self):
        ...
        with N_plate as batch_idx:
            with F_plate:
                ...
                x0 = pyro.sample("x0", self.Location(0., (((self.D+3)/(2*0.5))**2 - 1), -(self.D+3)/2, self.D+3))
                y0 = pyro.sample("y0", self.Location(0., (((self.D+3)/(2*0.5))**2 - 1), -(self.D+3)/2, self.D+3))

                junk_height = pyro.sample("junk_height", dist.HalfNormal(500.))
                junk_width = pyro.sample("junk_width", dist.Gamma(1, 0.1))
                junk_x0 = pyro.sample("junk_x0", self.Location(0., 2.5, -(self.D+3)/2, self.D+3))
                junk_y0 = pyro.sample("junk_y0", self.Location(0., 2.5, -(self.D+3)/2, self.D+3))
    ...
    def guide(self):
        ...
        #w_loc = pyro.param("w_loc", 1.5*torch.ones(self.N,self.F,1,1), constraint=constraints.positive)
        #w_beta = pyro.param("w_beta", 100*torch.ones(self.N,self.F,1,1), constraint=constraints.positive)
        ...
        #size = pyro.param("size", 1000 * torch.ones(self.N,self.F,1,1), constraint=constraints.greater_than(2.))
        ...
        #jw_beta = pyro.param("jw_beta", 100*torch.ones(self.N,self.F,1,1), constraint=constraints.positive)
        #jh_loc = pyro.param("jh_loc", self.data.height.reshape(self.N,self.F,1,1), constraint=constraints.positive)
        ...
        #jx_size = pyro.param("jx_size", 3. * torch.ones(self.N,self.F,1,1), constraint=constraints.greater_than(2.))
        #jsize = pyro.param("jsize", 3 * torch.ones(self.N,self.F,1,1), constraint=constraints.greater_than(2.))
        #jy_mode = pyro.param("jy_mode", torch.zeros(self.N,self.F,1,1), constraint=constraints.interval(-(self.D+3)/2,(self.D+3)/2))
        #jy_size = pyro.param("jy_size", 3. * torch.ones(self.N,self.F,1,1), constraint=constraints.greater_than(2.))
    ...
    def save(self, verbose=True):
        self.optim.save(os.path.join(self.data.path, "runs", "{}".format(self.dataset), "junks", "optimizer"))
        pyro.get_param_store().save(os.path.join(self.data.path, "runs", "{}".format(self.dataset), "junks", "params"))
        np.savetxt(os.path.join(self.data.path, "runs", "{}".format(self.dataset), "junks", "epoch_count"), np.array([self.epoch_count]))
        if verbose:
            print("Features were extracted and saved in {}.".format(self.data.path))

    def load(self):
        self.epoch_count = int(np.loadtxt(os.path.join(self.data.path, "runs", "{}".format(self.dataset), "junks", "epoch_count")))
        self.optim.load(os.path.join(self.data.path, "runs", "{}".format(self.dataset), "junks", "optimizer"))
        pyro.get_param_store().load(os.path.join(self.data.path, "runs", "{}".format(self.dataset), "junks", "params"))
