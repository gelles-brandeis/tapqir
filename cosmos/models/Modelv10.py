...
class Modelv10:
        ...
        with N_plate as batch_idx:
            with F_plate:
                ...
                with K_plate:
                    ...
                    x0 = pyro.sample("x0", Location(torch.tensor(0.), x0_size, -(self.D+3)/2, self.D+3))
                    y0 = pyro.sample("y0", Location(torch.tensor(0.), y0_size, -(self.D+3)/2, self.D+3))
    ...
    def guide(self):
        ...
        w_mode = pyro.param("w_mode", torch.ones(self.K,self.N,self.F,1,1)*1.35, constraint=constraints.interval(0.5,3.))
        w_size = pyro.param("w_size", torch.ones(self.K,self.N,self.F,1,1)*100., constraint=constraints.greater_than(2.))
        ...
        intensity = torch.ones(self.K,self.N,self.F,1,1)*5.
        intensity[1] = 100.
        h_loc = pyro.param("h_loc", intensity, constraint=constraints.positive)
        #h_loc = pyro.param("h_loc", torch.ones(1)*2, constraint=constraints.positive)
        h_beta = pyro.param("h_beta", torch.ones(self.K,self.N,self.F,1,1), constraint=constraints.positive)
        x_mode = pyro.param("x_mode", torch.zeros(self.K,self.N,self.F,1,1), constraint=constraints.interval(-(self.D+3)/2,(self.D+3)/2))
        y_mode = pyro.param("y_mode", torch.zeros(self.K,self.N,self.F,1,1), constraint=constraints.interval(-(self.D+3)/2,(self.D+3)/2))
        #intensity = torch.ones(self.K,self.N,self.F,1,1)*(((self.D+3)/(2*0.5))**2 - 1)
        size = pyro.param("size", torch.ones(1), constraint=constraints.positive)
