...
def view_m_probs(aoi, data, frames, m, z, sp, labels, predictions, prefix):
    ...
    if m:
        ...
        for k in range(2):
            ax.plot(
                ...
                data.predictions["m"][n, f1:f2+1, k],
    ...
    if z:
        z_probs = z_probs_calc(
            pyro.param(f"{prefix}/m_probs")[:, n],
            pyro.param(f"{prefix}/theta_probs")[n]).squeeze()
        ax.plot(
            data.drift.index.values[f1:f2+1],
...
def view_aoi(data, aoi, frame, z, sp, labels, predictions, target, prefix):
    ...
    for i in range(10):
        try:
            ...
            if target:
                ax[0, i].plot(
                    data.target["x"].iloc[n] + data.drift[f"{prefix}x"].iloc[f+i] + 0.5,
                    data.target["y"].iloc[n] + data.drift[f"{prefix}y"].iloc[f+i] + 0.5,
...
def view_single_aoi(data, aoi, frame, z, sp, labels, target, acc, prefix):
    ...
    if target:
        ax[0].plot(
            data.target["x"].iloc[n] + data.drift[f"{prefix}x"].iloc[f] + 0.5,
            data.target["y"].iloc[n] + data.drift[f"{prefix}y"].iloc[f] + 0.5,
            "b+", markersize=10, mew=3, alpha=0.7)
    if z:
...
def view_globals(z, j, data):
    ...
    if z:
        theta_probs = theta_probs_calc(
            ...
            param(f"{prefix}/theta_probs")).squeeze()
        h = ax[0,0].hist(
                param(f"{prefix}/h_loc").squeeze().data.reshape(-1),
                weights=theta_probs.reshape(-1),
                bins=100, label="z", alpha=0.3)
        ax[0,1].hist(
                param(f"{prefix}/w_mode").squeeze().data.reshape(-1),
                weights=theta_probs.reshape(-1),
                bins=100, label="z", alpha=0.3)
        ax[1,0].hist(
                param(f"{prefix}/x_mode").squeeze().data.reshape(-1),
                weights=theta_probs.reshape(-1),
                bins=100, label="z", alpha=0.3)
        ax[1,1].hist(
                param(f"{prefix}/y_mode").squeeze().data.reshape(-1),
                weights=theta_probs.reshape(-1),
                bins=100, label="z", alpha=0.3)
    if j:
    ...
    #h = torch.linspace(param(f"{prefix}/h_loc").min().item(), param(f"{prefix}/h_loc").max().item(), 100)
